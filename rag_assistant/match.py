...
def main():
    ...
    parser.add_argument(
        "--artifacts", type=str, default="artifacts", help="Directory containing persistent FAISS index."
    )
    parser.add_argument("--config", type=str, default="config/scoring.yaml", help="Path to scoring config YAML.")
    parser.add_argument("--top-k-chunks", type=int, default=40, help="Number of candidate chunks to retrieve.")
    parser.add_argument("--top-k-jobs", type=int, default=5, help="Number of final ranked top jobs to output.")
    parser.add_argument(
        "--allow-fallback", action="store_true", help="Allow SHA-256 fallback if transformer model fails to load."
    )
    ...
    candidate_jobs = retriever.retrieve_candidate_jobs(
        resume_text, top_k_chunks=args.top_k_chunks, max_candidate_jobs=15
    )

    if not candidate_jobs:
        logger.warning("No matching job chunks found in vector index.")
        sys.exit(0)
    ...
    results = []
    for job_id, chunks in candidate_jobs.items():
        first_chunk = chunks[0]
        job_title = first_chunk.get("title", "Target Role")
        company = first_chunk.get("company", "Company")
        job_req_skills = list(first_chunk.get("required_skills", []))
        job_pref_skills = list(first_chunk.get("preferred_skills", []))
        ...
        match_metrics = ranker.compute_composite_score(
            ...
            preferred_skill_coverage=skill_comp["preferred_coverage"] if job_pref_skills else None,
            resume_text=resume_text,
    ...
    # Stage 2 Ranking: Sort by overall match score descending & select top-k jobs
    results.sort(key=lambda x: x["match_metrics"]["overall_match_score"], reverse=True)
    final_top_results = results[: args.top_k_jobs]

    print("\n========================================================")
    print(f"   RAG CAREER MATCHING ANALYSIS REPORT for {resume_data['file_name']}")
    print("========================================================\n")
    print(f"Model Encoder Used: {embedder.model_used}")
    print(f"Candidate Jobs Ranked: {len(results)} (Top {len(final_top_results)} Displayed)\n")
    ...
    for idx, res in enumerate(final_top_results, 1):
        ...
        print(f"Component Breakdown: {json.dumps(res['match_metrics']['component_breakdown'], indent=2)}")
        print(
            f"Matching Skills ({len(res['skill_comparison']['matching_skills'])}): {[m['skill'] for m in res['skill_comparison']['matching_skills']]}"
        )
        print(
            f"Missing Required Skills ({len(res['skill_comparison']['missing_required_skills'])}): {[m['skill'] for m in res['skill_comparison']['missing_required_skills']]}"
        )
