...
Portfolio-grade end-to-end Retrieval-Augmented Generation (RAG) and Applied NLP system for matching resumes against job descriptions, identifying categorized skill gaps with sentence evidence, scoring candidate compatibility via a transparent 5-component matrix, and generating grounded career recommendations with job chunk citations.
...
Generated by running the automated benchmark evaluation suite (`evaluate.py`) on a synthetic dataset of 50 structured job descriptions and 10 candidate profiles across 10 role categories:

| Category | Metric | Score | Description |
| :--- | :--- | :---: | :--- |
| **Retrieval** | `Recall@5` | **0.9333** | Fraction of relevant target jobs in top-5 retrieved chunks |
| **Retrieval** | `Precision@5` | **0.3133** | Precision of top-5 retrieved job chunks |
| **Retrieval** | `MRR` | **0.8367** | Mean Reciprocal Rank of first relevant job result |
| **Retrieval** | `NDCG@5` | **0.8131** | Normalized Discounted Cumulative Gain ranking quality |
| **Skill Extraction** | `Precision` | **0.9781** | Precision of canonical skill alias extraction |
| **Skill Extraction** | `Recall` | **0.8552** | Coverage of ground-truth candidate skills |
| **Skill Extraction** | `F1 Score` | **0.9085** | Harmonic mean of skill extraction precision & recall |
