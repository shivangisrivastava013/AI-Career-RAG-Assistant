...
st.set_page_config(
    page_title="AI Career RAG Assistant",
    page_icon="🎯",
    layout="wide",
...
def main():
    ...
    st.markdown('<div class="sub-header">Production-grade Retrieval-Augmented Generation & Skill Alignment Engine</div>', unsafe_allow_html=True)
    ...
    top_k = st.sidebar.slider("Top-K Retrieved Chunks", min_value=1, max_value=15, value=5)
    allow_fallback = st.sidebar.checkbox("Allow SHA-256 Fallback", value=True, help="Explicitly allow SHA-256 feature encoding if transformer model fails to load.")
        ...
        if uploaded_file is not None:
            ...
            if os.path.exists(sample_txt_path):
                with open(sample_txt_path, "r", encoding="utf-8") as f:
                    resume_text = f.read()
                st.info("Using default sample resume (`data/resumes/sample_resume.txt`). Upload your own resume above to test!")

        if resume_text:
            if len(vstore.metadata_store) == 0:
                st.warning("No jobs indexed in FAISS vector store yet. Please go to Tab 2 ('Job Corpus Indexer') to build the index!")
            else:
                retriever = SemanticJobRetriever(vstore, embedder)
                chunks = retriever.retrieve_top_chunks(resume_text, top_k=top_k)
                grouped = retriever.group_chunks_by_job(chunks)
                ranker = ConfigurableJobRanker(config_path=config_file)
                rec_engine = GroundedRecommendationEngine()

                eval_results = []
                for job_id, j_chunks in grouped.items():
                    first_c = j_chunks[0]
                    j_title = first_c.get("title", "Target Position")
                    j_company = first_c.get("company", "Company")
                    j_text = " ".join([c["text"] for c in j_chunks])
                    j_req_skills = list(set([c.strip() for c in j_text.split() if len(c) > 2]))
                ...
                st.subheader(f"Ranked Compatibility Results ({len(eval_results)} Jobs)")
                for idx, item in enumerate(eval_results, 1):
                    score = item["metrics"]["overall_match_score"]
                    with st.container():
                        col1, col2 = st.columns([3, 1])
                        with col1:
                            st.markdown(f"### #{idx} {item['title']} @ **{item['company']}**")
                            st.markdown(f"**Location / Job ID:** `{item['job_id']}`")
                        with col2:
                        ...
                        b_cols = st.columns(5)
                        b_cols[0].metric("Required Skills", f"{bd['required_skill_coverage']}%")
                        b_cols[1].metric("Responsibilities", f"{bd['responsibility_semantic_similarity']}%")
                        b_cols[2].metric("Experience", f"{bd['experience_alignment']}%")
                        b_cols[3].metric("Education", f"{bd['education_alignment']}%")
                        b_cols[4].metric("Preferred", f"{bd['preferred_skill_coverage']}%")
    ...
    with tab2:
        st.subheader("2. Index Job Corpus into FAISS Vector Database")
        st.write("Ingest job description JSON records, extract section chunks, and persist FAISS index (`artifacts/jobs.faiss`).")
        ...
        if st.button("🚀 Rebuild FAISS Vector Index Now"):
            with st.spinner("Chunking & Embedding Job Corpus..."):
                ...
                if not jobs:
                    ...
                    new_vstore = PersistentFAISSVectorStore(dimension=embeddings.shape[1] if len(embeddings) > 0 else 384)
    ...
    with tab3:
        ...
        if "eval_results" in st.session_state and st.session_state["eval_results"]:
            ...
            sel_idx = st.selectbox("Select Target Job Role", range(len(job_titles)), format_func=lambda i: job_titles[i])
    ...
    with tab4:
        st.subheader("4. Grounded Resume Recommendations & Chunk Citations")
        if "eval_results" in st.session_state and st.session_state["eval_results"]:
            res_list = st.session_state["eval_results"]
            sel_idx_rec = st.selectbox("Select Target Role for Recommendations", range(len(res_list)), format_func=lambda i: f"{res_list[i]['title']} @ {res_list[i]['company']}")
    ...
    # TAB 5: EXPORT REPORT
    with tab5:
        st.subheader("5. Export Comprehensive Analysis Report")
        if "eval_results" in st.session_state and st.session_state["eval_results"]:
            res_list = st.session_state["eval_results"]
            export_payload = {
            ...
            st.download_button("📥 Download Analysis Report (JSON)", data=json_str, file_name="rag_career_analysis.json", mime="application/json")
        else:
